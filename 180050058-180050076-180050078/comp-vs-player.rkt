...
(define (draw t)
  ...
  (define s (cond [(= state2 1) q]
                  [else (cond [(and (= nosol 0) (< score score2)) (place-image (text/font "Computer Won , Better Luck Next time" 30 "black" "Gill Sans" "decorative" "italic" "normal" #f) 330 620 q)]
                              [(and (= nosol 0) (> score score2)) (place-image (text/font "Congratulations!! You won!" 30 "black" "Gill Sans" "decorative" "italic" "normal" #f) 330 620 q)]
                              [else (place-image (text/font "It's a tie" 30 "black" "Gill Sans" "decorative" "italic" "normal" #f) 330 620 q)])]))
 (place-image (circle 20 100 "red")  (pos-x (car t)) (pos-y (car t))
              (place-image (text (string-append "Score:" (number->string score)) 20 "black") 330 10 s)))
                            ...
                            (cond [(coincide? posn) t]
                                  [(box-coincide? posn u) (let* ([b (which-box posn u)]
                                                            ...
                                                            (if (or (hit? po e) (coincide? po )) t
                                                               ...
                                                               (begin (set! score (+ 1 score)) (list posn (append (list po) e) (caddr t)))))]
                               [else (begin (set! score (+ 1 score)) (list posn (cadr t) (caddr t)))]))]
    [(key=? a-key " ") (begin  (set! score +inf.0)  t)]
    [else t]))]
        [(= state 2) (cond 
...
(define (done? t)
  (cond [(= fl 1) #t]
        [(= count 0) #f] 
        [else     (let ([m (makelist boxes count one '())])
                 (if (or (= score +inf.0) (andmap (lambda (x) (check x m)) (cadr t))) (begin (set! fl 1) #f)
      #f))]))
...
;(displayln score)
;(cond [(< score score2) "Computer Won , Better Luck Next time"]
;      [(> score score2) "Congrats, You won "]
;      [else "It's  a tie"])
...
(display "Computer score : ")
(displayln score)
(cond [(and (= nosol 0) (< score score2)) "Computer Won , Better Luck Next time"]
      [(and (= nosol 0) (> score score2)) "Congrats, You won "]
      [else "It's  a tie"])
))
